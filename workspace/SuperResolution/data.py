...
class DataSet(object):
    def __init__(self, images_list_path, lr_images_path, num_epoch, batch_size):
    # filling the record_list
        ...
        image = tf.image.decode_jpeg(image_file, 1)
        ...
        lr_inputs = tf.image.decode_jpeg(image_file_lr, 1)
    ...
    def getTestData(self,lr_test_path):
        input_file = open(lr_test_path, 'r')
        
        self.record_test = []
        
        for line in input_file:
            line = line.strip()
            self.record_test.append(line)
        filename_queue = tf.train.string_input_producer(self.record_list,shuffle=False)
        image_reader = tf.WholeFileReader()
        _, image_file = image_reader.read(filename_queue)
        lr_inputs = tf.image.decode_jpeg(image_file, 1)
        
        
        lr_image = tf.image.resize_images(lr_inputs, [32, 32])
        lr_test_images = tf.train.batch([lr_image], batch_size=32)
        
        
        return lr_test_images
