...
class Net(object):
  ...
  def prior_network(self, hr_images):
    ...
    with tf.variable_scope('prior') as scope:
      conv1 = conv2d(hr_images, 64, [9, 9], strides=[1, 1], mask_type='A', scope="conv1")
      inputs = conv1
      state = conv1
      for i in range(20):
        inputs, state = gated_conv2d(inputs, state, [7, 7], scope='gated' + str(i))
      conv2 = conv2d(inputs, 1024, [1, 1], strides=[1, 1], mask_type='B', scope="conv2")
      conv2 = tf.nn.relu(conv2)
      prior_logits = conv2d(conv2, 1 * 256, [1, 1], strides=[1, 1], mask_type='B', scope="conv3")
      ...
      prior_logits = tf.concat([prior_logits[:, :, :, 0::1]], 1)
  ...
  def conditioning_network(self, lr_images):
    ...
    with tf.variable_scope('conditioning') as scope:
      ...
      for i in range(1):
        ...
        #inputs = deconv2d(inputs, 32, [3, 3], strides=[2, 2], scope="deconv" + str(i)) #original
        inputs = deconv2d(inputs, 32, [3, 3], strides=[1, 1], scope="deconv" + str(i))
      ...
      conditioning_logits = conv2d(inputs, 1*256, [1, 1], strides=[1, 1], mask_type=None, scope="conv")
