...
class Solver(object):
  def __init__(self):
    ...
    with tf.device(device_str):
      #dataset
      self.dataset = DataSet(conf.imgs_list_path, conf.lr_imgs_list_path, self.num_epoch, self.batch_size)
  ...
  def sample_test(self, sess, mu=1.1):
    c_logits = self.net.conditioning_logits   
    p_logits = self.net.prior_logits
    lr_imgs = self.dataset.lr_test_images       
    ...
    np_lr_imgs = np.reshape(random_ims,[32,32,32,1])
    ...
    for i in range(32):
      for j in range(32):
        for c in range(1):
        #for c in range(3):
          np_p_logits = sess.run(p_logits, feed_dict={gen_hr_imgs})
    ...
    for i in range(32):
      for j in range(32):
        ...
        #for c in range(3):
          ...
          gen_hr_imgs[:, i, j, c] = new_pixel
    #
    save_samples(np_lr_imgs, self.samples_dir +'/lr_imsgs'+ '/lr_' + str(mu*10) + '_' + str(step))
    save_samples(np_hr_imgs, self.samples_dir +'/hr_imsgs'+ '/hr_' + str(mu*10) + '_' + str(step))
    save_samples(gen_hr_imgs, self.samples_dir+'/gen_hr_imsgs' + '/generate_' + str(mu*10) + '_' + str(step))
