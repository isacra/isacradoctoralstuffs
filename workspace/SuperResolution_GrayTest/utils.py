...
def save_samples(np_imgs, img_path):
  ...
  num = int(N ** (0.5))
  merge_img = np.zeros((num * H, num * W, 1), dtype=np.uint8)
  #merge_img = np.zeros((num * H, num * W, 3), dtype=np.uint8)
  for i in range(num):
    for j in range(num):
      merge_img[i*H:(i+1)*H, j*W:(j+1)*W, :] = np_imgs[i*num+j,:,:,:]
      imsave(img_path+'_'+str(i)+str(j)+'.jpg', np_imgs[i*num+j,:,:,:]) #Save each image separately as a file
      
  imsave(img_path+'.jpg', merge_img)
