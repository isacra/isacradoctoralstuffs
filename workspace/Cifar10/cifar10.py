...
tf.app.flags.DEFINE_string('data_dir', '/home/malik/workspace/Cifar10/data',
...
def inference(images):
  ...
  with tf.variable_scope('conv2') as scope:
    ...
    bias = tf.nn.bias_add(conv, biases)
    conv2 = tf.tanh(bias, name=scope.name)
  ...
  with tf.variable_scope('local3') as scope:
    ...
    local3 = tf.add(tf.matmul(reshape, weights), biases)
...
#     local6 = tf.add(tf.matmul(local5, weights), biases, name=scope.name)
...
def loss(logits, labels):
  ...
  l = tf.reduce_mean(tf.pow((logits-labels),2))
...
def train(total_loss, global_step):
  ...
  with tf.control_dependencies([loss_averages_op]):
    opt = tf.train.GradientDescentOptimizer(lr).minimize(total_loss)
