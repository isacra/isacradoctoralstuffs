function[impFiltrada2] = highPassFilter2(signal, Ts, Ncoef, cutFreq)
% highPassFilter2(signal, Ts, Ncoef, cutFreq)
...
%
% Exemple  dado_filtrado = highPassFilter2(signal, 4, 110, 8);

Ts = Ts*1e-3;

for j=1:size(signal,2)
    [impFiltrada2(:,j),h] = highPassFilter(signal(:,j), Ts, Ncoef, cutFreq);
end
% figure
% plot(abs(fft(h,250)));
end 

function[impFiltrada,h] = highPassFilter(signal, Ts, Ncoef, cutFreq)
