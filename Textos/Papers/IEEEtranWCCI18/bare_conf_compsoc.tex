...
In recent years, considerable effort has been employed in single image
\cite{babacan,} and multi-image \cite{} blind deconvolution. 

[6, 14, 7, 9, 20, 2]single image
[4, 3, 13, 22] multi-image
...
the image from the optimization while estimating the unknown blur.
The proposal shows that it is possible to define a classe of prior images
suitable enough for representing sharp images features. Based on the prior
formulation, it is possible to use Bayesian inference for estimation of the of
the unknown image and the blur kernel.
...
The inversion modeling refers to using the current measurements of observable
...
m = F^{-1}(d)
\end{equation}
where, is the investigated physical system, and relates a set of model parameters
$m=(m_1, m_2,...,m_n)\subset R^n$ estimated through the observed data $d \in R^s$.
Frequently, in inversion problems some \textit{a priori} data is incorporated to the model.
...
The offshore seismic data is the main observable data used in seismic inversion. Its acquisition is performed
sending pulses to under water surface through a controlled artificial source and capturing the
...
on which the wavelet reflects. This characteristic is called reflectivity coefficient and it is
calculated as
\begin{equation}
...
wavelets have their band-width generally limited from $6Hz$ to $65Hz$. As a consequence,
the images resulting from the seismic inversion will have their frequency spectrum limited.

CNN can remedy this problem by estimating the numerous kernels, which are convolved along the
training images. T
...
\bibitem{levin}			A. Levin, Y. Weiss, F. Durand, W. T. Freeman, ``Understanding and evaluating blind deconvolution algorithms.''  In CVPR, 2009, p. 1964–1971.
\end{thebibliography}
