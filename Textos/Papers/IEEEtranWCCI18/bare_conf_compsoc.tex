...
on a set of synthetic acoustic impedance image. Using synthetic models
allows studying the results of the algorithms without external interferencies,
what leading to efficient interpretations and assessments. 
According to \cite{Harvey}, wedge shaped models is a straight way to analyse the
seismic model modeling and inversion processes. It realisticaly reproduces
reservoir contexts such as statigraphic refinements, \textit{pich-outs} in sand layers, 
edges and channel structures.

In order to generate a set of training images, we procceed two steps to obtain the wedge images.
...
was filling the lithologies with values of petrophysical properties.
